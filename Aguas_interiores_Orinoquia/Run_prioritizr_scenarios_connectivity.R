...
# Costos como integridad
# Categorias basadas en cuartiles
breaks <- c(0, 0.00062, 0.000809, 0.001105, 0.001821, Inf)
labels <- rev(c(10, 8, 6, 4, 2))  # Corresponding values for each class
...
if (F) {
  ...
  tester_end_time - tester_start_time
  
  plot(s1['solution_1'], main = "Solución de prueba") 
}

# 4. PARÁMETROS USUARIO---------------------------------------------------------
targets <- seq(0.1, 0.6, 0.1) 
penalties <- seq(0, 100, 20) 
...
write_log <- function(log_path, escenario, start_time, end_time, summary_table, 
  ...
  cat("Total soluciones:", ifelse(is.null(summary_table), 0, 
                                  nrow(summary_table)), "\n\n")
...
run_scenario <- function(escenario, p, t) {
  ...
  p1 <- problem(conectividad, rast(raster_stack), cost_column = "reclassified") %>%
    ...
    add_cbc_solver(verbose = FALSE) %>%
    add_binary_decisions() %>%
    add_linear_penalties(penalty = p, data = 'Int_ttl.r')
